...
// Colors matched to each project's theme
const ACCENTS = [
  { bar: "from-amber-400 via-orange-400 to-yellow-400",   glow: "rgba(251,191,36,0.22)" },
  { bar: "from-emerald-400 via-green-400 to-lime-400",    glow: "rgba(52,211,153,0.22)" },
  { bar: "from-blue-500 via-sky-400 to-cyan-400",         glow: "rgba(56,189,248,0.22)" },
  { bar: "from-violet-500 via-purple-400 to-fuchsia-400", glow: "rgba(167,139,250,0.22)" },
];

function useTilt(glow: string) {
  ...
  function onMove(e: React.MouseEvent<HTMLElement>) {
    ...
    el.style.transform = `perspective(700px) rotateY(${x * 12}deg) rotateX(${-y * 12}deg) scale(1.02)`;
...
function ProjectCard({ project, accent }: { project: typeof projects[number]; accent: typeof ACCENTS[number] }) {
  const { ref, onMove, onLeave } = useTilt(accent.glow);

  return (
    <motion.article
      ref={ref as React.RefObject<HTMLElement>}
      className="group flex flex-col bg-white/3 border border-white/10 rounded-2xl overflow-hidden hover:border-white/22 transition-colors duration-300 h-full"
      onMouseMove={onMove}
